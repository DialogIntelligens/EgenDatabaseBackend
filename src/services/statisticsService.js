...
export async function getGreetingRateService(query, pool) {
  const { chatbot_id, start_date, end_date } = query;
  if (!chatbot_id) return { statusCode: 400, payload: { error: 'chatbot_id is required' } };
  ...
  try {
    ...
    if (!start_date || !end_date) {
      ...
      if (!firstOpenDate) {
        return { statusCode: 200, payload: { total_opens: 0, total_conversations: 0, greeting_rate_percentage: 0, note: 'No chatbot opens tracked yet' } };
      }
    ...
    let note = null;
    if (totalOpens > 0) {
      greetingRate = Math.min(Math.round((totalConversations / totalOpens) * 100), 100);

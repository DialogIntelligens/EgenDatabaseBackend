...
app.get('/conversation/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      if (context === 'livechat') {
        // Mark as viewed when accessed from livechat page (affects all conversation types)
        await pool.query('UPDATE conversations SET viewed = TRUE WHERE id = $1', [id]);
      } else {
        // Only mark non-livechat conversations as viewed when accessed from regular conversations page
        // This prevents livechat conversations from being marked as read on the livechat page when viewed elsewhere
        await pool.query('UPDATE conversations SET viewed = TRUE WHERE id = $1 AND (is_livechat = FALSE OR is_livechat IS NULL)', [id]);
      }
    }
...
/*
  GET /api/shopify/settings/:chatbot_id
  Fetches Shopify configuration for a specific chatbot
*/
app.get('/api/shopify/settings/:chatbot_id', async (req, res) => {
  try {
    const { chatbot_id } = req.params;
    ...
    // Fetch Shopify settings for this chatbot
    const result = await pool.query(
      'SELECT * FROM chatbot_shopify_settings WHERE chatbot_id = $1',
      [chatbot_id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ 
        error: 'Shopify settings not found for this chatbot',
    ...
    return res.json({
      ...
      shopify_secret_key: settings.shopify_secret_key,
...
/*
  PUT /api/shopify/settings/:chatbot_id
  Updates Shopify configuration for a specific chatbot (admin only)
*/
app.put('/api/shopify/settings/:chatbot_id', authenticateToken, async (req, res) => {
  try {
    // Only admins can update Shopify settings
    if (!req.user.isAdmin) {
      return res.status(403).json({ error: 'Forbidden: Admin access required' });
    }
    ...
    // Upsert Shopify settings
    const result = await pool.query(
      `INSERT INTO chatbot_shopify_settings (
        chatbot_id, 
        shopify_enabled, 
        shopify_store, 
        shopify_access_token, 
